...
| `MATCH_REVIEW_FLOOR` | `0.5` | Lower bound for the `UNCERTAIN` review band; below this scores route to `UNMATCHED` |
...
| Category | Variables |
| --- | --- |
| LLM | `ANTHROPIC_API_KEY`, `LLM_MODE`, `SONNET_MODEL`, `HAIKU_MODEL`, `OPUS_MODEL`, `INGESTION_CONCURRENCY`, `INGESTION_MAX_RETRIES`, `MATCHING_CONCURRENCY`, `LLM_MAX_RETRIES`, `LLM_RETRY_BASE_SECONDS`, `LLM_RETRY_TPM_BASE_SECONDS` |
...
| Reconciliation | `BASE_CURRENCY`, `FX_VARIANCE_BUFFER_PCT`, `MATCH_CONFIDENCE_THRESHOLD`, `MATCH_REVIEW_FLOOR`, `EXTRACTION_ESCALATION_THRESHOLD`, `DATE_WINDOW_DAYS` |
