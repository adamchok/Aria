          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-3">
              <p className="text-sm text-slate-600">
                Force-create a batch job from all buffered transactions now, bypassing the scheduled
                thresholds. Requires uncleared ledger entries on a registered bank account.
              </p>
              {flushed && (
                <p className="text-sm font-medium text-emerald-700" role="status">
                  Batch job queued successfully.{' '}
                  <Link to="/jobs" className="underline hover:text-emerald-800">
                    View jobs
                  </Link>
                </p>
              )}
              {flushMutation.isError && (
                <p className="text-sm text-rose-600" role="alert">
                  Flush failed.{' '}
                  {flushMutation.error instanceof Error ? flushMutation.error.message : 'Unknown error.'}
                </p>
              )}
              <div>
                <Button
