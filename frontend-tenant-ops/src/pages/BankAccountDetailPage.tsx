...
function EditAccountModal({
  ...
  const save = useMutation({
    ...
    onSuccess: () => { invalidateAccountQueries(qc, accountId); onClose(); },
    onError: (e: Error) => setError(e.message),
  });
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-xl">
        <h2 id="edit-account-title" className="mb-4 text-lg font-semibold text-slate-900">Edit bank account</h2>
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg rounded-lg border border-slate-200 bg-white p-6 shadow-xl">
        <h2 id="edit-entry-title" className="mb-4 text-lg font-semibold text-slate-900">Edit ledger entry</h2>

        <div className="grid gap-4 sm:grid-cols-2">
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700">
            Value date
            <input type="date" value={form.value_date} onChange={field('value_date')} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700">
            Currency
            <input value={form.currency} onChange={field('currency')} maxLength={3} className="rounded-md border border-slate-300 px-3 py-2 text-sm uppercase" />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700 sm:col-span-2">
            Amount
            <input value={form.amount} onChange={field('amount')} inputMode="decimal" className="rounded-md border border-slate-300 px-3 py-2 text-sm tabular-nums" />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700 sm:col-span-2">
            Description
            <input value={form.description} onChange={field('description')} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700">
            Reference
            <input value={form.reference} onChange={field('reference')} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700">
            Counterparty
            <input value={form.counterparty} onChange={field('counterparty')} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </label>
        </div>
...
function AddLedgerEntryModal({
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg rounded-lg border border-slate-200 bg-white p-6 shadow-xl">
        <h2 id="add-entry-title" className="mb-4 text-lg font-semibold text-slate-900">Add ledger entry</h2>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700 sm:col-span-2">
            Amount
            <input value={form.amount} onChange={field('amount')} inputMode="decimal" placeholder="e.g. -250.00 for debit" className="rounded-md border border-slate-300 px-3 py-2 text-sm tabular-nums" />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700 sm:col-span-2">
            Description
            <input value={form.description ?? ''} onChange={field('description')} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          ...
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700">
            Reference
            <input value={form.reference ?? ''} onChange={(e) => setForm((f) => ({ ...f, reference: e.target.value || null }))} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </label>
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700">
            Counterparty
            <input value={form.counterparty ?? ''} onChange={(e) => setForm((f) => ({ ...f, counterparty: e.target.value || null }))} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </label>
        </div>
        ...
        <div className="mt-6 flex justify-end gap-2">
          <Button variant="secondary" onClick={onClose}>Cancel</Button>
          <Button loading={create.isPending} disabled={!form.value_date || !form.amount || !form.currency} onClick={() => create.mutate()}>
            Add entry
          </Button>
        </div>
      </div>
  ...
  return (
    <>
      <tr className="border-t border-slate-100 hover:bg-slate-50">
        <td className="cursor-pointer py-3 pl-4 pr-3 text-xs text-slate-500" onClick={() => setExpanded((e) => !e)}>
          {formatDate(entry.value_date)}
        </td>
        <td className="cursor-pointer px-3 py-3 text-sm tabular-nums text-slate-900 text-right" onClick={() => setExpanded((e) => !e)}>
          {formatAmount(entry.amount, entry.currency)}
        </td>
        <td className="cursor-pointer px-3 py-3 text-sm text-slate-700 max-w-xs truncate" onClick={() => setExpanded((e) => !e)}>
          {entry.description || '—'}
        </td>
        <td className="cursor-pointer px-3 py-3 text-xs text-slate-500" onClick={() => setExpanded((e) => !e)}>
          {entry.reference ?? '—'}
        </td>
        <td className="py-3 pl-3 pr-2 text-center">
          <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${entry.cleared ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
            {entry.cleared ? 'Cleared' : 'Pending'}
          </span>
        </td>
        <td className="py-3 pl-2 pr-4 text-right">
          {entry.cleared ? (
            <span className="text-xs text-slate-400" title="Cleared entries cannot be edited">Locked</span>
          ) : (
            <div className="flex justify-end gap-1">
              <Button size="sm" variant="secondary" aria-label={`Edit entry ${entry.reference ?? entry.id.slice(0, 8)}`} onClick={() => onEdit(entry)}>Edit</Button>
              <Button size="sm" variant="danger" aria-label={`Delete entry ${entry.reference ?? entry.id.slice(0, 8)}`} onClick={() => onDelete(entry)}>Delete</Button>
            </div>
          )}
        </td>
      </tr>
...
export function BankAccountDetailPage() {
  ...
  const deleteEntry = useMutation({
    ...
    onSuccess: () => { invalidateAccountQueries(qc, accountId!); setDeletingEntry(null); },
  });

  const deleteStatement = useMutation({
    mutationFn: (statementId: UUID) => api.deleteAccountStatement(accountId!, statementId),
    onSuccess: () => { invalidateAccountQueries(qc, accountId!); setDeletingStatementId(null); },
  });
  ...
  if (accError || !account) {
    return (
      <div className="flex flex-col items-center gap-3 p-12 text-center">
        ...
        <Link to="/bank-accounts"><Button variant="secondary">Back to accounts</Button></Link>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Uncleared', value: account.uncleared_count, highlight: account.uncleared_count > 0 },
        ].map((s) => (
          <Card key={s.label}>
            <CardContent className="py-4 text-center">
              <p className={`tabular-nums text-3xl font-semibold ${s.highlight ? 'text-amber-700' : 'text-slate-900'}`}>{s.value}</p>
          ...
          <div className="flex gap-1">
            {CLEARED_TABS.map((f) => (
              <button
                ...
                className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${clearedFilter === f.value ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
              >
                {f.label}
              </button>
            ))}
            ...
            <CardContent className="p-0">
              {ledgerLoading && (
                <div className="space-y-2 p-4">
                  {[...Array(8)].map((_, i) => <div key={i} className="h-10 animate-pulse rounded bg-slate-100" />)}
                </div>
              )}
              {!ledgerLoading && ledger?.items.length === 0 && (
                <p className="p-8 text-center text-sm text-slate-500">
                  {clearedFilter === 'all' ? 'No entries yet. Upload a statement or add entries manually.' : `No ${clearedFilter} entries.`}
                </p>
              )}
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody>
                        {ledger.items.map((entry) => (
                          <LedgerRow key={entry.id} entry={entry} onEdit={setEditingEntry} onDelete={setDeletingEntry} />
                        ))}
                      </tbody>
                    </table>
                  </div>
      ...
      {activeTab === 'statements' && (
        <Card>
          ...
          <CardContent className="p-0">
            {stmtsLoading && (
              <div className="space-y-2 p-4">
                {[...Array(4)].map((_, i) => <div key={i} className="h-12 animate-pulse rounded bg-slate-100" />)}
              </div>
            )}
                    ...
                    {statements.map((s) => (
                      <tr key={s.id} className="border-t border-slate-100 hover:bg-slate-50">
                        ...
                        <td className="px-3 py-3 text-sm tabular-nums text-right">
                          <span className={s.uncleared_count > 0 ? 'text-amber-700' : 'text-emerald-700'}>{s.uncleared_count}</span>
                        </td>
                        <td className="px-3 py-3 text-xs text-slate-500">{formatDate(s.created_at.slice(0, 10))}</td>
                        <td className="py-3 pl-3 pr-4 text-right">
                          <Button size="sm" variant="danger" aria-label={`Delete statement ${s.filename}`} onClick={() => setDeletingStatementId(s.id)}>Delete</Button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      {showUpload && <UploadStatementModal accountId={accountId!} onClose={() => setShowUpload(false)} />}

      {showEditAccount && account && (
        <EditAccountModal accountId={accountId!} current={account} onClose={() => setShowEditAccount(false)} />
      )}

      {showAddEntry && account && (
        <AddLedgerEntryModal accountId={accountId!} defaultCurrency={account.currency} onClose={() => setShowAddEntry(false)} />
      )}

      {editingEntry && (
        <EditLedgerEntryModal accountId={accountId!} entry={editingEntry} onClose={() => setEditingEntry(null)} />
      )}

      {deletingEntry && (
        <ConfirmDialog
          title="Delete ledger entry?"
          message={<>Remove <strong>{deletingEntry.description || deletingEntry.reference || 'this entry'}</strong> ({formatAmount(deletingEntry.amount, deletingEntry.currency)}) from the ledger? This cannot be undone.</>}
      ...
      {deletingStatement && (
        <ConfirmDialog
          ...
          message={<>Delete <strong>{deletingStatement.filename}</strong> and all {deletingStatement.entry_count} ledger {deletingStatement.entry_count === 1 ? 'entry' : 'entries'}? Cleared entries will also be removed.</>}
      ...
      {showDeleteAccountConfirm && (
        <ConfirmDialog
          ...
          message={<>This will permanently delete <strong>{account.name}</strong> and all its statements and ledger entries. This cannot be undone.</>}
